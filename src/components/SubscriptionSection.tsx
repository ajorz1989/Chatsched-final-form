...
export default function SubscriptionSection({ userId, role }: Props) {
  ...
  async function subscribe() {
    ...
    if (invokeError || data?.error) {
      setError(formatSupabaseError(invokeError || data?.error, "Couldn't start the subscription"));
      return;
    }
  ...
  async function cancelSubscription() {
    ...
    if (invokeError || data?.error) {
      setError(formatSupabaseError(invokeError || data?.error, "Couldn't cancel subscription"));
      return;
    }
