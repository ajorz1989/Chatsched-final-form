...
export default function PortfolioManager({ publisher, onChange }: { publisher: Publisher; onChange: () => void }) {
  ...
  async function saveVideo() {
    ...
    setSavingVideo(false);
    if (error) {
      setVideoError("Couldn't save that — try again in a moment.");
      return;
    }
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (uploadErr) {
      setUploading(false);
      // The bucket's own file_size_limit/allowed_mime_types (schema_phase27)
      // can still reject something that slipped past the checks above —
      // e.g. a file whose real size differs from what the browser reported.
      setUploadError("Upload failed — that file may be too large or an unsupported type.");
      return;
    }
    ...
    setUploading(false);
    if (updateErr) {
      setUploadError("Uploaded, but couldn't add it to your profile — try again.");
      return;
    }
    invalidatePublishersCache();
    onChange();
