...
export default function AccountSettings() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      reportError(err, { source: "AccountSettings.export" });
      setExportError("Couldn't put your export together — try again in a moment.");
    } finally {
      setExporting(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    if (error || data?.error) {
      if (data?.blockers) {
        ...
        setDeleteError(data?.error ?? "Couldn't delete your account — try again in a moment.");
        reportError(error ?? data?.error, { source: "AccountSettings.delete" });
      }
      return;
    }
    await signOut();
